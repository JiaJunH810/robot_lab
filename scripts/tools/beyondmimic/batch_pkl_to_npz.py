...
class MotionLoader:
    ...
    def _load_motion(self):
        ...
        print(f"Motion loaded ({self.motion_file}), duration: {self.duration} sec, frames: {self.input_frames}", flush=True)
...
def run_simulator(motion_file, sim: sim_utils.SimulationContext, scene: InteractiveScene):
    ...
    while simulation_app.is_running():
        ...
        # set joint state
        # 根据IsaacLab算出joint_pos, joint_vel这些值?
        # 但是后面又进行了赋值？
...
def main():
    ...
    for motion in tqdm(motions):
        print("当前异常运动序列个数", len(except_motion), flush=True)
        try:
            rel_path = os.path.relpath(motion, start=args_cli.input_file)
            rel_path_npz = os.path.splitext(rel_path)[0] + ".npz"
            output_root = "/home/ubuntu/projects/GMR/all_motion_data/waist1_npz"
            full_output_path = os.path.join(output_root, rel_path_npz)
            output_dir = os.path.dirname(full_output_path)
            os.makedirs(output_dir, exist_ok=True)
            args_cli.output_name = full_output_path
            print(args_cli.output_name, flush=True)
            sim.reset()
            run_simulator(motion, sim, scene)
        except Exception as e:
            except_motion.append(motion)

# python scripts/tools/beyondmimic/batch_pkl_to_npz.py -f /home/ubuntu/projects/GMR/all_motion_data/waist1 --input_fps 30 --headless
