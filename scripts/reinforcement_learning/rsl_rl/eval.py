...
from robot_lab.utils.exporter import attach_onnx_metadata, export_vae_policy_as_jit, export_vae_policy_as_onnx
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlBaseRunnerCfg):
    ...
    motion_cmd = env.unwrapped.command_manager._terms["motion"]
    motion_loader = motion_cmd.motion
    num_motions = motion_loader.num_motions
    
    for m_id in tqdm(range(num_motions)):
        motion_name = os.path.basename(motion_loader.files[m_id])
        motion_len = motion_loader.time_step_total[m_id].item()
        env_ids = torch.arange(env_cfg.scene.num_envs, device=env.device)
        motion_cmd.motion_ids[env_ids] = m_id
        motion_cmd.time_steps[env_ids] = 0
        # motion_cmd._resample_command(env_ids)

        obs, _ = env.get_observations(), env.reset()

        for t in range(motion_len):
            with torch.inference_mode():
                actions = policy(obs)
                obs, _, dones, _ = env.step(actions)
                if torch.any(dones): break
