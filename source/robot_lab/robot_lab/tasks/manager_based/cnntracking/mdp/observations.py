...
from robot_lab.tasks.manager_based.cnntracking.mdp.commands import MotionCommand
...
def future_obs(env: ManagerBasedEnv, command_name:str, horizon: int, attr: str) -> torch.Tensor:
    command: MotionCommand = env.command_manager.get_term(command_name)
    obs = command.get_future_obs(horizon=horizon, attr=attr)
    return obs