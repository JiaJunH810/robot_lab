...
class MotionLoader:
    def __init__(self, motion_file: str, code_book: str, body_indexes: Sequence[int], anchor_index: int, future_body_indexes: Sequence[int], device: str = "cpu"):
        ...
        code_data = np.load(code_book, allow_pickle=True)
        self.codebook = torch.tensor(code_data['codebook'], dtype=torch.float, device=device)
        self.anchor_index = anchor_index
        self.future_body_indexes = future_body_indexes
        ...
        for motion_file in self.files:
            if motion_file not in code_data:
                raise ValueError(f"Motion file '{motion_file}' found on disk but NOT in codebook file!")
            
            self.code_idx.append(torch.tensor(code_data[motion_file], dtype=torch.long, device=device))
            ...
            if self.code_idx[-1].shape[0] != self.joint_pos[-1].shape[0]:
                raise ValueError(f"Motion file '{motion_file}' of length is unequal!")
        ...
        self.code_idx = torch.cat(self.code_idx, dim=0)
    ...
    def anchor_pos_w(self) -> torch.Tensor:
        return self._body_pos_w[:, self.anchor_index]

    @property
    def anchor_quat_w(self) -> torch.Tensor:
        return self._body_quat_w[:, self.anchor_index]
    
    @property
    def anchor_lin_vel_w(self) -> torch.Tensor:
        return self._body_lin_vel_w[:, self.anchor_index]
    
    @property
    def future_body_pos_w(self) -> torch.Tensor:
        return self._body_pos_w[:, self.future_body_indexes]
...
class MotionCommand(CommandTerm):
    ...
    def __init__(self, cfg: MotionCommandCfg, env: ManagerBasedRLEnv):
        ...
        self.future_body_indexes = torch.tensor(
            self.robot.find_bodies(self.cfg.future_body_names, preserve_order=True)[0], dtype=torch.long, device=self.device
        )

        self.motion = MotionLoader(self.cfg.motion_file, self.cfg.code_book,self.body_indexes, self.robot_anchor_body_index, self.future_body_indexes, device=self.device)
    ...
    def robot_whole_com_pos_w(self) -> torch.Tensor:
        ...
        return torch.sum(self.masses * body_com_pos, dim=1) / torch.sum(self.masses, dim=1)
    
    @property
    def vqvae_code(self) -> torch.Tensor:
        code_indexes = self.motion.code_idx[self._current_steps]
        return torch.nn.functional.embedding(code_indexes.long(), self.motion.codebook)
    
    def get_future_obs(self, attr: str, horizon: int = 5) -> torch.Tensor:
        offsets = torch.arange(0, horizon * 5, 5, device=self.device)
        timesteps_local = self.time_steps.unsqueeze(1) + offsets.unsqueeze(0)
        timesteps_local = torch.minimum(timesteps_local, self.motion.time_step_total[self.motion_ids].unsqueeze(1) - 1)
        starts = self.motion.motion_starts[self.motion_ids].unsqueeze(1)
        timesteps_global = starts + timesteps_local
        data = getattr(self.motion, attr)
        return data[timesteps_global]
    ...
    def _adaptive_sampling(self, env_ids: Sequence[int]):
        ...
        # Sample Motion
        if self.cfg.use_motion_adaptive_sampling:
        ...
        if self.cfg.use_adaptive_sampling:
            ...
            sampling_probabilities = torch.ones((len(env_ids), self.bin_count), device=self.device) / float(self.bin_count)
            sampled_bins = torch.multinomial(sampling_probabilities, 1).squeeze(-1)
...
class MotionCommandCfg(CommandTermCfg):
    ...
    use_motion_adaptive_sampling: bool = True
