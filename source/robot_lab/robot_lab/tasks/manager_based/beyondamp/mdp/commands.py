...
class MotionCommand(CommandTerm):
    ...
    def _adaptive_sampling(self, env_ids: Sequence[int]):
        ...
        # Metrics
        H = -(sampling_probabilities * (sampling_probabilities + 1e-12).log()).sum(dim=1)
    ...
    def _update_command(self):
        self.time_steps += 1
        env_ids = torch.where(self.time_steps >= self.motion.time_step_total[self.motion_ids])[0]
    ...
    def sample_expert_transition(self) -> tuple[torch.Tensor, torch.Tensor]:
        motion_lengths = self.motion.time_step_total[self.motion_ids]
        starts = self.motion.motion_starts[self.motion_ids]
        t_local = torch.rand(self.num_envs, device=self.device) * (motion_lengths - 1).float()
        t_local = t_local.long().clamp(max=motion_lengths - 2)
        t_global = starts + t_local
        next_t_global = starts + t_local + 1
