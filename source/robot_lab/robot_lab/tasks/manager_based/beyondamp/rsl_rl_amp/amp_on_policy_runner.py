...
class AMPOnPolicyRunner:
        ...
        for it in range(start_it, total_it):
            ...
            with torch.inference_mode():
                for _ in range(self.cfg["num_steps_per_env"]):
                    # Sample AMP expert datas
                    motion_cmd = self.env.unwrapped.command_manager.get_term("motion")
                    expert_states, expert_next_states = motion_cmd.sample_expert_transition()
                    # Sample actions
                    actions = self.alg.act(obs, amp_obs)
                    # Step the environment
                    obs, rewards, dones, extras, reset_env_ids, terminal_amp_states = self.env.step(actions.to(self.env.device))
                    ...
                    next_amp_obs_with_terminal = torch.clone(next_amp_obs)
                    next_amp_obs_with_terminal[reset_env_ids] = terminal_amp_states
                    lerp_rewards, d_logits, amp_rewards = self.alg.discriminator.predict_amp_reward(
                        amp_obs, next_amp_obs_with_terminal, rewards, normalizer=self.alg.amp_normalizer)
                    amp_obs = torch.clone(next_amp_obs)                    
                    # Process the step
                    self.alg.process_env_step(obs, lerp_rewards, dones, extras, next_amp_obs_with_terminal, expert_states, expert_next_states)
    ...
    def construct_algorithm(
        ...
        # Compute per-action min_std scaled by joint range (matching beyondAMP)
        amp_min_normalized_std = cfg.get("amp_min_normalized_std", 0.0)
        if amp_min_normalized_std > 0:
            joint_limits = env.unwrapped.scene["robot"].data.joint_pos_limits[0, :env.num_actions]
            joint_range = torch.abs(joint_limits[:, 1] - joint_limits[:, 0])
            amp_min_normalized_std = amp_min_normalized_std * joint_range.to(device)
