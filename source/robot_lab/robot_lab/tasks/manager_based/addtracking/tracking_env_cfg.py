...
import robot_lab.tasks.manager_based.addtracking.mdp as mdp
...
class CommandsCfg:
    ...
    motion = mdp.MotionCommandCfg(
        asset_name="robot",
        resampling_time_range=(1.0e9, 1.0e9),   # IsaacLab中自带超时时间，这个就是范围，手动调成无限大
        debug_vis=True,
        pose_range={
            "x": (-0.05 * random_scale, 0.05 * random_scale),
            "y": (-0.05 * random_scale, 0.05 * random_scale),
            "z": (-0.01, 0.01),
            "roll": (-0.1 * random_scale, 0.1 * random_scale),
            "pitch": (-0.1 * random_scale, 0.1 * random_scale),
            "yaw": (-0.2 * random_scale, 0.2 * random_scale),
        },
        velocity_range=VELOCITY_RANGE,
        joint_position_range=(-0.1 * random_scale, 0.1 * random_scale),
    )
...
class ObservationsCfg:
    ...
    class DiscriminatorCfg(ObsGroup):
        root_pos_diff = ObsTerm(func=mdp.root_pos_diff, params={"command_name": "motion"})
        root_quat_diff = ObsTerm(func=mdp.root_quat_diff, params={"command_name": "motion"})
        root_lin_vel_diff = ObsTerm(func=mdp.root_lin_vel_diff, params={"command_name": "motion"})
        root_ang_vel_diff = ObsTerm(func=mdp.root_ang_vel_diff, params={"command_name": "motion"})
        joint_pos_diff = ObsTerm(func=mdp.joint_pos_diff, params={"command_name": "motion"})
        joint_vel_diff = ObsTerm(func=mdp.joint_vel_diff, params={"command_name": "motion"})
        body_pos_diff = ObsTerm(func=mdp.body_pos_diff, params={"command_name": "motion"})
...
class SafetyRewardsCfg:
    ...
    #             "contact_forces",
    #             body_names=[
    #                 r"^(?!left_ankle_roll_link$)(?!right_wrist_roll_rubber_hand$)(?!left_wrist_roll_rubber_hand$).+$"
    #             ],
    #         ),
    #         "threshold": 1.0,
    #     },
