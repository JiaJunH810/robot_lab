...
class LoggerAdd:
    ...
    def __init__(
        ...
        self.drewbuffer = deque(maxlen=100)
        self.trewbuffer = deque(maxlen=100)
        ...
        self.cur_dreward_sum = torch.zeros(self.num_envs, dtype=torch.float, device=self.device)
        self.cur_treward_sum = torch.zeros(self.num_envs, dtype=torch.float, device=self.device)
    ...
    def process_env_step(
        ...
        dones: torch.Tensor,
        extras: dict,
        disc_rewards: torch.Tensor | None = None,
        ...
        if self.log_dir is not None:
            ...
            if disc_rewards is not None:
                self.cur_dreward_sum += disc_rewards
                self.cur_treward_sum += task_rewards
            ...
            if disc_rewards is not None:
                self.drewbuffer.extend(self.cur_dreward_sum[new_ids][:, 0].cpu().numpy().tolist())
                self.cur_dreward_sum[new_ids] = 0
                self.trewbuffer.extend(self.cur_treward_sum[new_ids][:, 0].cpu().numpy().tolist())
                self.cur_treward_sum[new_ids] = 0
    ...
    def log(
        ...
        if self.log_dir is not None and not self.disable_logs:
            ...
            if len(self.rewbuffer) > 0:
                ...
                if len(self.drewbuffer) > 0:
                    log_string += f"""{"Mean disc reward:":>{pad}} {statistics.mean(self.drewbuffer):.2f}\n"""
                if len(self.trewbuffer) > 0:
                    log_string += f"""{"Mean task reward:":>{pad}} {statistics.mean(self.trewbuffer):.2f}\n"""
