...
from rsl_rl_elastic.algorithms import PPOElastic
...
class OnPolicyRunnerElastic:
        ...
        for it in range(start_it, total_it):
            ...
            with torch.inference_mode():
                for _ in range(self.cfg["num_steps_per_env"]):
                    ...
                    # update elastic
                    elastic_gt = self.env.unwrapped.command_manager._terms["motion"].elastic_gt

                    # Move to device
                    obs, rewards, dones = (obs.to(self.device), rewards.to(self.device), dones.to(self.device))
                    # Process the step
                    self.alg.process_env_step(obs, rewards, dones, extras, elastic_obs, elastic_gt)
    ...
    def _construct_algorithm(self, obs: TensorDict) -> PPOElastic:
        ...
        actor_critic: ActorCriticElastic = actor_critic_class(
        ...
        alg: PPOElastic = alg_class(
            actor_critic, elastic, storage, device=self.device, **self.alg_cfg, multi_gpu_cfg=self.multi_gpu_cfg
        )
