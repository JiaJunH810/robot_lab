    ...
    def __init__(
        ...
        **kwargs: dict[str, Any],
        ...
        for obs_group in obs_groups["policy"]:
            ...
            num_actor_essence += obs[obs_group].shape[-1]
        num_critic_essence = 0
        for obs_group in obs_groups["critic"]:
            assert len(obs[obs_group].shape) == 2, "The ActorCritic module only supports 1D observations."
            num_critic_essence += obs[obs_group].shape[-1]
        num_future = 0
        for obs_group in obs_groups["future"]:
            num_future += obs[obs_group].shape[-1]

        # Actor
        self.state_dependent_std = state_dependent_std
        self.actor = Actor(
            num_essence=num_actor_essence,
            num_future_encoder=128,
            num_action=num_actions,
            timesteps=20,
            input_channels=num_future,
            output_channels=[128, 64],
        ...
        # Critic
        self.critic = Critic(
            num_essence=num_critic_essence,
            num_future_encoder=128,
            timesteps=20,
            input_channels=num_future,
            output_channels=[128, 64],
