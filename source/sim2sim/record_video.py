...
class RobotLabSim2Sim:
    def __init__(self, xml_path, motion_file, policy_path, code_path):
        ...
        self.viewer = mujoco_viewer.MujocoViewer(self.m, self.d, mode='offscreen')
        self.viewer.cam.distance = 3.0

        video_fps = int(1.0 / (self.m.opt.timestep * 20)) 
        video_name = os.path.basename(motion_file).replace('.npz', '.mp4')
        self.video_writer = imageio.get_writer(video_name, fps=video_fps, macro_block_size=1)
        print(f"视频将保存为: {video_name}")
...
if __name__ == "__main__":
    ...
    motion_file = "/home/ubuntu/projects/hjj-robot_lab/source/motion/motion_hjq/up_down/113_08_poses.npz"
    ...
    r = RobotLabSim2Sim(xml_path, motion_file, policy_path, code_path)
    r.run()