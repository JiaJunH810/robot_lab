...
class RobotLabSim2Sim:
    ...
    def future_motion(self, timestep, horizon, anchor_name, extra_body_name):
        ...
        # current_anchor_pos = torch.tensor(self.motion_body_pos_w[timestep, anchor_idx]).unsqueeze(0).repeat(horizon * len(extra_body_name), 1)
        # current_anchor_quat = torch.tensor(self.motion_body_quat_w[timestep, anchor_idx]).unsqueeze(0).repeat(horizon * len(extra_body_name), 1)
        # extra_pos, _ = subtract_frame_transforms(
        #     current_anchor_pos,
        #     current_anchor_quat,
        #     future_extra_body_pos,
        #     None,
        # )
        # extra_pos = extra_pos.view(horizon, -1)
        return torch.cat([pos, ori, vel], dim=-1).flatten().numpy()
