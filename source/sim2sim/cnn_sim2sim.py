...
class RobotLabSim2Sim:
    ...
    def get_future_data(self, current_timestep, horizon, anchor_name):
        anchor_idx = self.lab_body_names.index(anchor_name)
        future_idx = np.arange(current_timestep + 1, current_timestep + horizon + 1) % self.motion_joint_pos.shape[0]
        joint_pos = self.motion_joint_pos[future_idx]
        joint_vel = self.motion_joint_vel[future_idx]
        anchor_lin_vel_w = self.motion_body_lin_vel_w[future_idx, anchor_idx]
        anchor_ang_vel_w = self.motion_body_ang_vel_w[future_idx, anchor_idx]
        future = np.concatenate([joint_pos, joint_vel, anchor_lin_vel_w, anchor_ang_vel_w], axis=-1)
    ...
    def run(self):
        ...
        for i in tqdm(range(int(sim_duration / sim_dt)), desc="Running simulation..."):
            ...
            if i % sim_decimation == 0:
                ...
                future = self.get_future_data(timestep, horizon, anchor_name)
...
if __name__ == "__main__":
    ...
    motion_file = "/home/ubuntu/projects/hjj-robot_lab/source/motion/G1_1waist_Run_TurnLeft_Walk.npz"
    policy_path = "/home/ubuntu/projects/hjj-robot_lab/logs/rsl_rl/unitree_g1_cnn_flat/2026-01-17_16-16-38/exported/policy.onnx"
